...
In the result, substrings will be in decreasing order of their length 
and when they have the same length sorted alphabetically (more precisely sorted by codepoint); 
...
def mix(s1, s2):
    ...
    s1_clean = [c for c in s1 if c in lower_case]
    s2_clean = [c for c in s2 if c in lower_case]
    
    s1_dict = dict((c,s1_clean.count(c)) for c in set(s1_clean))
    s2_dict = dict((c,s2_clean.count(c)) for c in set(s2_clean))

    # (arr_num,is_equal,letter,count)
    result = []
    
    for k,v in s1_dict.iteritems():
        if s2_dict.get(k):
            if max(v, s2_dict[k]) > 1:
                if v < s2_dict[k]:
                    result.append((2,False,k,-s2_dict[k]))
                elif v == s2_dict[k]:
                    result.append((3,True,k,-v))
                else:
                    result.append((1,False,k,-v))
        elif v > 1:
            result.append((1,False,k,-v))
    
    for k,v in s2_dict.iteritems():
        if (not s1_dict.get(k)) and v > 1:
            result.append((2,False,k,-v))
        
    return "/".join(sort_and_format(result))
    
def sort_and_format(arr):
    formatted = []
    arr = sorted(arr, key=itemgetter(3,0,2))
    for n,e,k,v in arr:
        if n == 3:
            n = "="
        formatted.append("{}:{}".format(n,k*(-v)))

    return formatted


# More concise solution using Counter library
